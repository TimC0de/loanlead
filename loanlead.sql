...
CREATE TABLE `branches` (
  ...
  KEY `FKki9w25xskrhvval4oqwj62k8b` (`entity_id`),
  CONSTRAINT `branches_entities_id_fk` FOREIGN KEY (`entity_id`) REFERENCES `entities` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
...
CREATE TABLE `entities` (
  ...
  `created_at` datetime NOT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `entities_name_uindex` (`name`)
...
--
-- Table structure for table `loans_state`
--

DROP TABLE IF EXISTS `loans_state`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `loans_state` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
...
--
-- Dumping data for table `loans_state`
--

LOCK TABLES `loans_state` WRITE;
/*!40000 ALTER TABLE `loans_state` DISABLE KEYS */;
/*!40000 ALTER TABLE `loans_state` ENABLE KEYS */;
UNLOCK TABLES;
...
CREATE TABLE `users` (
  ...
  KEY `users_ibfk_6` (`role_id`),
  KEY `users_branches_id_fk` (`branch_id`),
  KEY `users_phone_numbers_id_fk` (`phone_numbers_id`),
  CONSTRAINT `users_branches_id_fk` FOREIGN KEY (`branch_id`) REFERENCES `branches` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `users_ibfk_6` FOREIGN KEY (`role_id`) REFERENCES `roles` (`name`) ON DELETE CASCADE ON UPDATE CASCADE,
...
BEGIN
    ...
    WHERE lp.loan_product = l.loan_product AND
          ...
          ls.loan_id = loan_id AND ls.actioned_by = u.employee_id AND u.role_id = r.name;
...
BEGIN
    ...
    DECLARE loans_cursor CURSOR FOR SELECT id FROM previous_loanlead.loans;
...
BEGIN
    ...
    INSERT INTO entities(id, name, short_name, box_number, plot_number, branches_number, description, logo_path, created_at)

    SELECT e.id, e.name, e.short_name, e.box_number, e.plot_number, e.branches_number, e.description, e.logo_path, e.created_at
    ...
    INSERT INTO branches(id, entity_id, name, type, district, town, created_at)

    SELECT b.id, b.entity_id, b.name, b.type, b.district, b.town, b.created_at
    ...
    INSERT INTO users(id, employee_id, role_id, branch_id, phone_numbers_id, password, full_name, email, status, picture_path, receive_sms, updated_at, status_change_timestamp, created_at)

    SELECT u.id, CONCAT(u.employee_id, u.id), r.name, u.branch_id, p.id, u.password, u.full_name, u.email, u.status, u.picture_path, u.receive_sms, u.updated_at, CURRENT_TIMESTAMP, u.created_at

    FROM previous_loanlead.users AS u,

         previous_loanlead.roles AS r,

         phone_numbers AS p

    WHERE r.id = u.role_id AND p.first_phone_number = u.phone_number1 AND p.second_phone_number = u.phone_number2;
    ...
    INSERT INTO loans(id, customer_id, loan_product, amount, tenure, created_at, receive_timestamp, defer_stage, type_changed)

    SELECT l.id, l.customer_id, lp.loan_product, l.amount, l.tenure, l.created_at, l.receive_timestamp, l.stage_deferred, l.type_changed

    FROM previous_loanlead.loans AS l,

         loan_products AS lp

    WHERE lp.id = l.loan_threshold_id;
    ...
    SELECT a.id, a.loan_id, a.status, a.comment, a.created_at, u.employee_id
    ...
    DELETE FROM loans_state;



    INSERT INTO loans_state(id, actioned_by, status, comment, actioned_at, staged_at)

    SELECT l.id, u.employee_id, l.status, l.comment, l.receive_timestamp, l.stage_timestamp

    FROM previous_loanlead.loans AS l,

         users AS u

    WHERE u.id = l.staged_by;
...
-- Dump completed on 2019-06-24  2:16:44
